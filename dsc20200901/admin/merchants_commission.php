...
function merchants_commission_list()
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT mis.*, msf.companyName, msf.company_adress, msf.company_contactTel FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' as mis ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('merchants_steps_fields') . ' as msf on mis.user_id = msf.user_id ' . $where . ' ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'];
