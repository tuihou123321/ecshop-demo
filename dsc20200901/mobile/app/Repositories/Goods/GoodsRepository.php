...
class GoodsRepository implements \App\Contracts\Repositories\Goods\GoodsRepositoryInterface
{
	...
	public function findByType($type = 'best', $size = 10)
	{
		...
		$goods = \App\Models\Goods::select('goods_id', 'cat_id', 'user_cat', 'user_id', 'goods_sn', 'goods_name', 'click_count', 'brand_id', 'provider_name', 'goods_number', 'market_price', 'shop_price', 'promote_price', 'goods_thumb', 'goods_img', 'original_img')->where($type, 1)->where('is_on_sale', 1)->where('is_delete', 0)->orderby('goods_id', 'desc')->limit($size)->get()->toArray();
		return $goods;
	}

	public function goodsInfo($id)
	{
		$res = \App\Models\Goods::select('goods_id', 'goods_name', 'shop_price as goods_price', 'market_price', 'goods_number as stock', 'goods_desc', 'desc_mobile', 'goods_brief', 'sales_volume as sales', 'goods_thumb', 'model_attr', 'goods_type', 'user_id', 'is_on_sale', 'promote_price', 'product_price', 'product_promote_price', 'promote_start_date', 'promote_end_date')->where('goods_id', $id)->where('is_delete', 0)->first();
	...
	public function getBrandNameByGoodsId($goodsId)
	{
		...
		return !empty($brandName) ? $brandName[0] : '';
	}
