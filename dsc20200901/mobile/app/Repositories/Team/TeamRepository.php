...
class TeamRepository implements \App\Contracts\Repositories\Team\TeamRepositoryInterface
{
	protected $goods;
	private $field;
	private $authService;
	...
	{
		return \App\Models\OrderInfo::select('*')->where('team_id', $team_id)->where('extension_code', 'team_buy')->where('pay_status', 2)->orWhere('order_status', 4)->count();
	}
	...
	{
		$begin = ($page - 1) * $size;
		$list = \App\Models\OrderInfo::from('order_info as o')->select('o.add_time', 'o.team_id', 'o.user_id', 'o.team_parent_id', 'o.team_user_id')->leftjoin('users as u', 'o.user_id', '=', 'u.user_id')->where('o.team_id', $team_id)->where('o.extension_code', 'team_buy')->where('o.pay_status', 2)->orWhere('o.order_status', 4)->orderby('o.add_time', 'asc')->offset($begin)->limit($size)->get()->toArray();
		return $list;
	}
