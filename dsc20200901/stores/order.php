...
function store_order_list($ru_id = 0, $store_id = 0)
{
	...
	$arr = array();
	$sql = 'SELECT so.*, o.order_id, o.order_sn, o.add_time, o.consignee, o.order_status, o.shipping_status, o.pay_status, so.store_id,so.take_time,o.mobile, ' . '(' . order_amount_field('o.') . ') AS total_fee ' . 'FROM ' . $GLOBALS['ecs']->table('store_order') . ' AS so, ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . $where . ' AND o.order_id = so.order_id ' . 'ORDER by ' . $filter['sort_by'] . ' ' . $filter['sort_order'];
