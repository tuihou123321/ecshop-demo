...
<div class="floor-line-con floorFive <?php echo $this->_var['spec_attr']['typeColor']; ?>" data-idx="1" id="floor_module_<?php echo $this->_var['spec_attr']['floorMode']; ?>" ectype="floorItem">
    <div class="floor-hd" ectype="floorTit">
        <?php if ($this->_var['spec_attr']['floor_title'] || $this->_var['spec_attr']['cat_name']): ?><div class="hd-tit"><?php if ($this->_var['spec_attr']['style_icon'] != 'other'): ?><i class="iconfont icon-<?php echo $this->_var['spec_attr']['style_icon']; ?>"></i><?php else: ?><i class="iconfont"><img src="<?php echo $this->_var['spec_attr']['cat_icon']; ?>" alt="分类图标"></i><?php endif; ?><em class="iconfont icon-spot"></em><?php if ($this->_var['spec_attr']['floor_title']): ?><?php echo $this->_var['spec_attr']['floor_title']; ?><?php elseif ($this->_var['spec_attr']['cat_name']): ?><?php echo $this->_var['spec_attr']['cat_name']; ?><?php else: ?>主分类名称<?php endif; ?></div><?php endif; ?>
        <div class="hd-tags">
            <ul>
                <li class="first current">新品推荐</li>
                <?php if ($this->_var['spec_attr']['cateValue']): ?>
...
<div class="video_box">
    <div id="video_upload_area">
        <div class="video_notice">支持上传时长在10秒(10MB)以内的视频，请勿上传色情、反动等违法视频</div>
    ...
    <div class="layer_point" style="display: none;">
        <dl id="video_loading" class="point clearfix" style="display: none;">
            ...
            <dd>
                <p id="updesc" class="updesc">视频上传中...</p>
                <p class="S_txt2"><label id="uploading_tip">上传速度取决于您的网速，请耐心等待。</label><a href="javascript:void(0);" id="video_upload_cancel" class="video_upload_cancel" onclick="video_upload_cancel()">取消上传</a></p>
            </dd>
        </dl>
    ...
    <div class="upload_video_area disabled" id="upload_video_area" ectype="upload_video_area">
        <a id="upload_video" class="btn btn30 blue_btn mt10" href="javascript:void(0);">开始上传</a>
    </div>
</div>
<script type="text/javascript">
    var uploader_video = new plupload.Uploader({//创建实例的构造方法
        ...
        init:{
            ...
            FileUploaded: function(up, file, info) { //文件上传成功的时候触发
				...
				if(data.error == 1){
					$("#upload_succ").html("视频上传失败");
				}else if(data.error == 2){
					$("#upload_succ").html("视频上传格式有误");
				}else{
					$("#upload_succ").html("视频上传成功");
				}
