			...
			if ($_REQUEST['act'] == 'insert_link_desc' || $_REQUEST['act'] == 'update_link_desc') {
				...
				if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
					...
					if (!empty($goods_sn)) {
						...
						if (!empty($goods_sn)) {
							$sql = 'SELECT product_id FROM ' . $ecs->table('products') . ' AS p,' . $ecs->table('goods') . ' AS g' . ('WHERE p.product_sn = \'' . $goods_sn . '\' AND p.goods_id = g.goods_id AND g.user_id = \'') . $adminru['ru_id'] . '\'';
