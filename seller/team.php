...
function list_link($is_add = true)
{
	...
	return array('href' => $href, 'text' => "商品列表", 'class' => 'icon-reply');
}
...
if ($_REQUEST['act'] == 'list') {
	...
	$smarty->assign('action_link', array('href' => 'team.php?act=add', 'text' => "添加拼团商品", 'class' => 'icon-plus'));
	$tab_menu = array();
	$tab_menu[] = array('curr' => 1, 'text' => "拼团商品列表", 'href' => 'team.php?act=list');
	...
	if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
		...
		if ($_REQUEST['act'] == 'edit') {
			$smarty->assign('ur_here', "编辑商品");
		}
		else {
			$smarty->assign('ur_here', "拼团商品");
		}
		...
		$smarty->assign('team_list', team_cat_list(0, $team_list));//拼团频道树形
		...
		if (0 < $id) {
			...
			$sql = 'SELECT count(goods_id) as num  FROM ' . $GLOBALS['ecs']->table('team_goods') . ' WHERE goods_id = \'' . $goods['goods_id'] . '\'  ';
			$res = $GLOBALS['db']->getRow($sql);
